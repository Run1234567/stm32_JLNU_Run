...
本项目为 STM32 提供了一套统一的函数接口，底层分别基于**标准库**和**寄存器**实现。开发者无需修改业务代码，即可在“开发便捷性”与“极致性能”之间自由切换。

## ⚖️ 版本对比


| **版本**     | **寄存器版 (Register)**  | **标准库版 (Std-Lib)**  |
| ------------ | ------------------------ | ----------------------- |
| **底层实现** | 直接操作内存地址         | 调用 ST 官方标准库      |
| **执行效率** | ⭐⭐⭐⭐⭐ (极快)        | ⭐⭐⭐ (有函数调用开销) |
| **代码体积** | ⭐⭐⭐⭐⭐ (极小)        | ⭐⭐⭐ (包含库文件)     |
| **可读性**   | ⭐⭐ (位运算较多)        | ⭐⭐⭐⭐⭐ (逻辑清晰)   |
| **适用场景** | 极致时序控制、Flash 受限 | 快速开发、常规业务      |
