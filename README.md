...
- [STM32 AT24C02 (EEPROM) 存储模块使用说明](#stm32-at24c02-(EEPROM)-驱动模块使用说明)
- [STM32 DS18B20 温度传感器驱动模块使用说明](#stm32-ds18b20-(Temperature-Sensor)-驱动模块使用说明)
...
本模块实现了对 MPU6050 寄存器的读写操作，提供了获取原始加速度、角速度和板载温度的接口。
...
驱动读取出来的 `int16_t` 是 ADC 的**原始数值 (Raw Data)**，需要结合初始化时的量程配置转换为物理单位。

### 4.1 加速度换算 (Accel)

本驱动默认配置量程为 \$\\pm 2g\$。

MPU6050 是 16位 ADC，\$\\pm 2g\$ 对应灵敏度为 16384 LSB/g。

\$\$\\text{加速度 (g)} = \\frac{\\text{RawValue}}{16384.0}\$\$

* *示例*: 如果读到 `ax = 16384`，代表 X 轴受力 1g (静止平放时的重力)。

### 4.2 角速度换算 (Gyro)

本驱动默认配置量程为 \$\\pm 2000^\\circ/s\$ (最不灵敏，但量程最大)。

对应灵敏度为 16.4 LSB/(°/s)。

\$\$\\text{角速度 (deg/s)} = \\frac{\\text{RawValue}}{16.4}\$\$

* *示例*: 如果读到 `gx = 164`，代表 X 轴正在以 10度/秒 的速度旋转。

---

## 5. API 接口详解

### 5.1 初始化 `MPU6050_Init`
...
* **功能**: 初始化 I2C 总线，复位 MPU6050，唤醒传感器，并配置滤波和量程。
* **addr**: 设备地址。如果模块 AD0 引脚接地，填 `MPU_ADDR_DEFAULT` (0xD0)；如果接 VCC，填 `0xD2`。
...
* **注意**: 传入的是变量的**地址** (`&variable`)。
* **返回值**: 无直接返回值，数据通过指针写入。

### 5.3 获取温度 `MPU6050_Get_Temp`
...
* **功能**: 读取内部温度传感器。
* **返回**: 浮点型摄氏度 (e.g. `36.53`)。公式已内置：`Raw / 340.0 + 36.53`。

---

## 6. 常见问题 (Pitfalls)

### 6.1 ID 读取失败 (返回 0x00 或 0xFF)

1. **接线问题**: 检查 SCL/SDA 是否接反。
2. **上拉电阻**: 许多 MPU6050 模块自带上拉电阻，但如果你使用的是裸芯片或自制板子，确保 SCL 和 SDA 都有 4.7kΩ 上拉到 3.3V。
3. **地址错误**: 检查 AD0 引脚电平。如果代码写 `0xD0` 但 AD0 接了高电平，通讯会失败。

### 6.2 数据全是 0

* 检查是否调用了 `MPU6050_Init`。
* 初始化函数中包含了 `PWR_MGMT_1` 寄存器的写操作 (`0x00`)，这是**解除休眠**的关键。如果不执行这一步，芯片处于睡眠模式，不更新数据。

### 6.3 读数漂移 (Zero Drift)

* 消费级 MEMS 传感器都有零点漂移。即静止时，角速度不为 0 (例如显示 15)。
* **解决方法**: 在程序启动时（假设静止），读取 100 次数据求平均值，作为**Offset (偏移量)**。后续读取时，减去这个 Offset。

### 6.4 姿态解算 (DMP vs 原始数据)
...
**下一步**：如果您需要实现**文件系统** (如 FATFS) 来管理 Flash 中的文件，我可以为您提供 FATFS 的移植接口代码 (`diskio.c`)，将这些底层 API 对接到文件系统中。
